...
specifying the description of amino acids.
...
termination (stop) codon (In older versions, `X` was used instead.) In the Table below, to support
translation from a DNA sequence, we have used a `T` in the codons although in nature RNA is
translated so the codons contain `U`.
