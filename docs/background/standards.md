...
At the protein level HGVS nomenclature follows the _Nomenclature and Symbolism for Amino Acids and Peptides_ (see [IUPAC-IUB](http://www.iupac.org/publications/pac/1984/pdf/5605x0595.pdf)), specifying the description of amino acids. In addition HGVS nomenclature uses `Ter` (three-letter amino acid code) and `*` (three- and one-letter amino acid code) to indicate a translation termination (stop) codon (In older versions, `X` was used instead.) In the Table below, to support translation from a DNA sequence, we have used a `T` in the codons although in nature RNA is translated so the codons contain `U`.
